...
from MFTIQ.RAFT.core.utils.frame_utils import read_gen
import os
import logging
...
class TextAugmenter():
    ...
    def add_text(self, img1, img2, valid):
        ...
        img1_text = cv2.putText(copy.deepcopy(img1), text, (x_pos, y_pos), font, font_size, color,
                                thickness=font_thickness)
        img2_text = cv2.putText(copy.deepcopy(img2), text, (x_pos, y_pos), font, font_size, color,
                                thickness=font_thickness)
...
class FlowAugmenterDict:
    def __init__(self, crop_size, min_scale=-0.2, max_scale=0.5, do_flip=True, **kwargs):
        ...
        self.blend_prop = 0.5
        self.blend_aug = BlendAugmenter(source_dir=self.blend_source, blend_prop=self.blend_prop,
                                        do_blend_transform=self.do_blend_transform)
        ...
        self.add_text_aug = TextAugmenter(do_add_text=self.do_add_text_transform, max_add_text=self.max_add_text,
                                          add_text_prop=self.add_text_prop)
    ...
    def color_transform(self, data, prefix='img'):
        ...
        # symmetric
        image_stack = np.concatenate(values_list, axis=0)
        image_stack = np.array(self.photo_aug(Image.fromarray(image_stack)), dtype=np.uint8)
        values_list = np.split(image_stack, len(values_list), axis=0)

        # asymmetric
        if np.random.rand() < self.asymmetric_color_aug_prob:
            values_list = [np.array(self.asymetric_photo_aug(Image.fromarray(img)), dtype=np.uint8) for img in values_list]

        for idx, k in enumerate(keys_list):
            data[k] = values_list[idx]
        return data
    ...
    def spatial_transform(self, data):
        ...
        img_shape = einops.parse_shape(data[img_key], 'H W C')
        min_scale = np.maximum(
            (self.crop_size[0] + 8) / float(img_shape['H']),
            (self.crop_size[1] + 8) / float(img_shape['W']))
        ...
        if np.random.rand() < self.spatial_aug_prob:
            ...
            for k, v in data.items():
                # IMAGES
                if 'img' in k:
                    data[k] = cv2.resize(v, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
                # OCCLUSIONS
                elif 'occl' in k:
                    data[k] = cv2.resize(v, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
        ...
        img_shape_scaled = einops.parse_shape(data[img_key], 'H W C')
        y0 = np.random.randint(0, img_shape_scaled['H'] - self.crop_size[0])
        x0 = np.random.randint(0, img_shape_scaled['W'] - self.crop_size[1])

        for k, v in data.items():
            data[k] = v[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]].copy()
        return data


    def out_of_frame_occlusion_direct(self, flow, occl):
        occl_max_value = 1.0
        H, W, C = flow.shape
    ...
    def out_of_frame_occlusion(self, flow, data):
        occl_max_value = 1.0
        H, W, C = flow.shape
        x0, y0 = np.meshgrid(np.arange(W), np.arange(H))
        pos_x = flow[:, :, 0] + x0
        pos_y = flow[:, :, 1] + y0
        for k, occl in data.items():
            if 'occl' in k:
                occl[pos_x < 0] = occl_max_value
                occl[pos_y < 0] = occl_max_value
                occl[pos_x >= W] = occl_max_value
    ...
    def __call__(self, data, seed=None):
        with tmp_np_seed(seed):
            ...
            data = self.color_transform(data, prefix='img')

            has_occl = False
            has_valid = False
            for k in data.keys():
                if 'occl' in k and data[k] is not None:
                    has_occl = True
                if 'valid' in k and data[k] is not None:
                    has_valid = True

            if not has_occl:
                raise NotImplementedError('It is not used. Thus, it was not implemented for MFTIQ training')
                # data['img1'], data['img2'] = self.eraser_transform(data['img1'], data['img2'])

            if not has_valid:
                data['valid'] = (np.abs(data['flow'][:, :, 0]) < 1000) & (np.abs(data['flow'][:, :, 1]) < 1000)
                data['valid'] = np.expand_dims(data['valid'], axis=2).astype(float)

            data = self.spatial_transform(data)

            if self.do_jpeg_transform or self.do_blend_transform or self.do_add_text_transform:
                raise NotImplementedError('It is not used. Thus, it was not implemented for MFTIQ training')
                # data['img1'], data['img2'] = self.blend_aug(data['img1'], data['img2'])
                # data['img1'], data['img2'], data['valid'] = self.add_text_aug(data['img1'], data['img2'], data['valid'])
                # data['img1'], data['img2'] = self.jpeg_transform(data['img1'], data['img2'])

            if 'flow' in data:
                data = self.out_of_frame_occlusion(data['flow'], data)
            else:
                for k in data.keys():
                    if 'flow' in k and 'flow_est' not in k:
                        c_flow = data[k]
                        occl_key = k.replace('flow', 'occl')
                        c_occl = data[occl_key]
                        data[occl_key] = self.out_of_frame_occlusion_direct(c_flow, c_occl)
