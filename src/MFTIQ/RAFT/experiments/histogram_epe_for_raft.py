...
from RAFT.core.utils.statistics import EPEStatistics
...
def main(args):
    ...
    for dataset_name, data in datasets.items():
        ...
        epeStats = EPEStatistics(50, dataset_name, save_root=args.save_path)
        ...
        for idx in tqdm.trange(len(data)):
            ...
            flow_gt_np = flow_gt.detach().numpy()
            flow_pred_np = flow_pred.detach().cpu().numpy()
            flow_epe = np.sqrt(np.sum((flow_pred_np - flow_gt_np)**2, axis=0))

            epeStats.add_epe(flow_epe)
