...
def estimate_occlusion_by_depth_and_segment(
    ...
    return tf.logical_or(depth_occluded, tf.transpose(seg_occluded)), depth
...
def rotate_surface_normals(
    ...
    obj_normals = world_frame_normals_frames * cam_to_pt
    dots = tf.reduce_sum(obj_normals, axis=-1)
    faces_away = dots > 0
...
def single_object_reproject(
    ...
    occl_est, depth_map_from_est = estimate_occlusion_by_depth_and_segment(
            depth_map[:, :, :, 0],
            segments[:, :, :, 0],
    ...
    if quat is not None:
        faces_away, obj_normals = rotate_surface_normals(
            normals,
            world_pos,
            cam_positions,
            quat2rot(quat),
        ...
        obj_normals = tf.zeros([tf.shape(pt)[0], num_frames, 3], dtype=tf.float32)

    return obj_reproj, tf.logical_or(faces_away, obj_occ), depth_proj, tf.transpose(depth_map_from_est), obj_normals
...
def track_points(
    ...
    for i in range(max_seg_id):
        ...
        obj_reproj, obj_occ, depth_proj, depth_est, obj_norm = tf.cond(
            tf.shape(pt)[0] > 0,
            functools.partial(
        ...
        all_depths_est.append(depth_est)
        all_depths_proj.append(depth_proj)
        all_normals.append(obj_norm)
    ...
    all_depths_proj = tf.concat(all_depths_proj, axis=0)
    all_depths_est = tf.concat(all_depths_est, axis=0)
    all_normals = tf.concat(all_normals, axis=0)
    ...
    return (tf.cast(chosen_points, tf.float32), tf.cast(all_reproj, tf.float32), all_occ,
            tf.cast(all_depths_proj, tf.float32), tf.cast(all_depths_est, tf.float32),
            segmentations, tf.cast(all_normals, tf.float32))
...
def add_tracks(data,
    ...
    query_points, target_points, occluded, depth_proj, depth_est, segmentations, normals = track_points(
    ...
    depth_est.set_shape([None, num_frames])
    depth_proj.set_shape([None, num_frames])
    # segmentations = einops.rearrange(segmentations, 'B H W 1 -> (H W) B')
    # segmentations.set_shape([None, num_frames])
    normals.set_shape([None, num_frames, 3])
...
def save_flowou(rgb, target_points, query_points, occluded, file_num, split,
                save_root='/datagrid/personal/neoral/kubric_movi_e_longterm'):
    N, H, W, C = rgb.shape
...
def get_flow(data_name=None, data_dir=None, query_frame=None, target_frame=None):
    ds = tfds.as_numpy(create_point_tracking_dataset(
        data_dir='/datagrid/personal/neoral/kubric_longtermflow_dataset/',
...
def get_flow_from_points(rgb, target_points, query_points, occluded, depth_proj, depth_est, normals, segmentations):
    N, H, W, C = rgb.shape

    occls = np.zeros([H, W, N], dtype=bool)
    positions = np.zeros([H, W, N, 2], dtype=np.float32)

    depth_proj_all = np.zeros([H, W, N], dtype=np.float32)
    depth_est_all = np.zeros([H, W, N], dtype=np.float32)
    normals_all = np.zeros([H, W, N, 3], dtype=np.float32)
    ...
    depth_proj_all[query_points[:, 1], query_points[:, 2], :] = depth_proj[:, :]
    depth_est_all[query_points[:, 1], query_points[:, 2], :] = depth_est[:, :]
    normals_all[query_points[:, 1], query_points[:, 2], :, :] = normals[:, :, :]
    ...
    for frame_idx in range(N):
        ...
        c_depth_proj = depth_proj_all[:, :, frame_idx]
        c_depth_proj_reshape = einops.rearrange(c_depth_proj, 'H W -> 1 H W')
        depth_proj_list.append(c_depth_proj_reshape)

        c_depth_est = depth_est_all[:, :, frame_idx]
        c_depth_est_reshape = einops.rearrange(c_depth_est, 'H W -> 1 H W')
        depth_est_list.append(c_depth_est_reshape)

        c_normals = normals_all[:, :, frame_idx, :]
        c_normals_reshape = einops.rearrange(c_normals, 'H W xyz -> xyz H W', xyz=3)
        normals_list.append(c_normals_reshape)

        c_segmentation = segmentations[frame_idx]
        c_segmentation_reshape = einops.rearrange(c_segmentation, 'H W 1 -> 1 H W')
        segmentations_list.append(c_segmentation_reshape.numpy())

    return {'rgb': rgb_list, 'occlusion': occl_list, 'flow': flow_list,
            'depth_proj': depth_proj_list, 'depth_est': depth_est_list,
            'normals': normals_list, 'segmentations': segmentations_list}
...
def get_multiflow(dir_path, frames=None):
    ...
    Args:
        dir_path: location of custom kubric data directory
                    e.g. '/datagrid/personal/neoral/kubric_longtermflow_dataset/00715/RES_1024x1024__FPS_120__NFRAMES_240'
    ...
    return get_flow_from_points(data['video'], data['target_points'], data['query_points'], data['occluded'],
                                data['depth_proj'], data['depth_est'], data['normals'], data['segmentations'])
...
if __name__ == '__main__':
    ...
    # main(split='test')
    # get_flow()
    p = '/datagrid/personal/neoral/kubric_longtermflow_dataset/00715/RES_1024x1024__FPS_120__NFRAMES_240'
    # # p = '/datagrid/personal/neoral/kubric_longtermflow_dataset/00001/RES_256x256__FPS_12__NFRAMES_24'

    for i in range(5):
        start = time.time()
        outputs = get_multiflow(p, frames=[10, 100])
