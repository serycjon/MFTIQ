...
from MFTIQ.RAFT.core.raft_orig import RAFT_ORIG as RAFT
from MFTIQ.RAFT import evaluate
import MFTIQ.RAFT.core.datasets as datasets

from torch.utils.tensorboard import SummaryWriter

from MFTIQ.RAFT.core.utils.timer import Timer

from MFTIQ.RAFT.core.utils.flow_viz import flow_to_color
...
class Logger:
    def __init__(self, model, scheduler, logfile_comment='', logdir=None):
        self.model = model
    ...
    def _print_training_status(self):
        ...
        if self.writer is None:
            self.writer = SummaryWriter(log_dir=self.logdir, comment=self.logfile_comment)
    ...
    def write_images(self, inputs):
        ...
        for key in inputs:
            ...
            if key == 'valid':
                ...
                self.writer.add_images(key, torch.clip(im, 0., 255.).type(torch.uint8), dataformats='NCHW', global_step=self.total_steps)
            ...
            if (total_steps == 7) or (total_steps % VAL_FREQ == VAL_FREQ - 1):
                ...
                for val_dataset in args.validation:
                    if val_dataset == 'chairs':
                        ...
                        # print('val: ', train_timer.iter(), train_timer())
                    elif val_dataset == 'sintel_kubric_val_subsplit':
                        results.update(evaluate.validate_kubric(model.module, subsplit='validation'))
                        results.update(evaluate.validate_sintel(model.module, subsplit='validation'))
