...
def occlusion_loss(occl_est, occl_gt):
    ...
    loss = cross_ent_loss(torch.unsqueeze(occl_est.squeeze(dim=0).softmax(dim=0), dim=0), torch.unsqueeze(occl_gt_thresholded, dim=0))
    return torch.squeeze(loss).cpu()

def occlusion_accuracy(occl_est, occl_gt):
    assert len(occl_est.shape) == 3
    assert occl_est.shape == (2, occl_gt.shape[1], occl_gt.shape[2])
    pred = occl_est.softmax(dim=0)[1, :, :] > 0.5
    gt = occl_gt[0, :, :] > 0.5
