...
def generate_pairs_v2(dirname):
  img_pairs = []
  dirname = Path(dirname)
  paths = sorted(list(dirname.glob('*.png')) + list(dirname.glob('*.jpg')) + list(dirname.glob('*.jpeg')))
  for i in range(len(paths) - 1):
    img_pairs.append((str(paths[i]), str(paths[i + 1])))
  return img_pairs
...
if __name__ == '__main__':
    ...
    if args.eval_type == 'sintel':
        ...
        img_pairs = generate_pairs_v2(args.seq_dir)
    with torch.no_grad():
