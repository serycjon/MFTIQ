...
from MFTIQ.utils.various import with_debugger
...
def run(args):
    ...
    for pickle_path in tqdm.tqdm(dataset_conf.pickles, desc='pkl shards', position=0, leave=None, ascii=True):
        ...
        for seq in tqdm.tqdm(dataset, desc='sequences', position=1, leave=None, ascii=True):
            ...
            if getattr(args, 'seq', None) is not None and orig_sequence_name not in args.seq:
                continue
            ...
            for query_mode in tqdm.tqdm(query_modes, desc='query mode', position=2, leave=None, ascii=True):
                ...
                for tracker_config in tqdm.tqdm(configs, desc='trackers', position=3, leave=None, ascii=True):
                    ...
                    if not seq_querymode_tracker_result_path.exists():
                        logger.error(f"Results for {orig_sequence_name}, '{query_mode}' query mode not found @ {seq_querymode_tracker_result_path}")
                        continue
                    with open(seq_querymode_tracker_result_path, 'rb') as fin:
                        tracklet_outputs = pickle.load(fin)
                        pred_tracks = tracklet_outputs['tracks']  # (1, N_queries, N_frames, xy), scaled to 256 x 256
                        pred_occluded = tracklet_outputs['occluded']  # (1, N_queries, N_frames)
                    ...
                    assert gt_valid.shape == gt_occluded.shape
