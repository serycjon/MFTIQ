...
import MFTIQ.utils.geom_utils as gu
from MFTIQ.utils.rand_utils import tmp_np_seed
from MFTIQ.utils.interpolation import FlowInterpolator
...
def imshow(win_name, img, scalable=True,
    ...
    if export_video_path is not None:
        if win_name not in IMSHOW_WRITERS:
            IMSHOW_WRITERS[win_name] = VideoWriter(export_video_path, images_export=export_images)
        elif str(IMSHOW_WRITERS[win_name].path) != str(export_video_path):
            IMSHOW_WRITERS[win_name].close()
            IMSHOW_WRITERS[win_name] = VideoWriter(export_video_path, images_export=export_images)

        IMSHOW_WRITERS[win_name].write(img)
    if export_video_path is None or show_write:
...
def flush_video_writers():
    for writer in IMSHOW_WRITERS.values():
        writer.close()


# def flush_imshow_writers():
#     for name, writer in IMSHOW_WRITERS.items():
#         writer.close()
...
def vis_flow_dots(flow, src_img, dst_img, grid_sz=30,
    ...
    xs, ys = np.meshgrid(src_xs, src_ys, indexing='ij')
    flat_xs = xs.flatten()
    flat_ys = ys.flatten()
...
def cross(src, coords, color, radius=3, thickness=1):
    canvas = src.copy()
    vertical = np.array([0, 1])
    horizontal = np.array([1, 0])

    line(canvas, coords - vertical * radius, coords + vertical * radius, color, thickness)
    line(canvas, coords - horizontal * radius, coords + horizontal * radius, color, thickness)
