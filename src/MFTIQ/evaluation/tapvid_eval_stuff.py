...
from MFTIQ.utils.misc import parse_scale_WH
try:
...
def compute_tapvid_metrics(
    ...
    Args:
       ...
       query_mode: Either 'first' or 'strided', depending on how queries are
         ...
         evaluation.
       gt_valid: A boolean array of shape [b, n, t], where t is the number of frames.
                 False indicates that the point annotation is invalid (uncertain pose and visibility)
    ...
    if gt_valid is None:
        gt_valid = np.ones_like(gt_occluded) > 0
...
def sample_queries_strided(
    ...
    Args:
      ...
      target_valid: Boolean GT validity flag of shape [n_tracks, n_frames]
    ...
    if target_valid is None:
        target_valid = np.ones_like(target_occluded) > 0
    tracks = []
    occs = []
    valids = []
    queries = []
    ...
    for i in range(0, target_occluded.shape[1], query_stride):
        ...
        valids.append(target_valid[mask])
    ...
    return {
        ...
        "valid": np.concatenate(valids, axis=0)[np.newaxis, ...],
...
def sample_queries_first(
    ...
    target_valid = target_valid[valid, :]
    trackgroup = np.arange(target_occluded.shape[0])
...
def create_davis_dataset(
    ...
    for video_name in davis_points_dataset:
        ...
        raise NotImplementedError("Valid flag annotation not implemented here")
...
def create_tapvid_dataset(
        pickle_path, query_modes, train_size=None, fake_video=False, lazy_video=False, random_sample_size=None,
    ...
    if random_sample_size is not None and random_sample_size < N_sequences:
        current_random_state = random.getstate()
        random.seed(random_sample_seed)
        selected_tapvid_points_dataset = random.sample(list(tapvid_points_dataset.keys()), random_sample_size)
        random.setstate(current_random_state)
    else:
        selected_tapvid_points_dataset = tapvid_points_dataset.keys()

    for video_name in selected_tapvid_points_dataset:
        ...
        if "strided" in query_modes:
            converted["strided"] = sample_queries_strided(target_occ, target_points, frames,
                                                          target_valid=tapvid_points_dataset[video_name].get('valid'))
